<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="keywords" content="Cyber, Cybersecurity, Security, Hacking, Veiligheid, Internet, Online, Beveiliging">
    <meta name="description" content="Helpt organisaties bestendiger te maken tegen cyber dreigingen">
    <meta name="author" content="Lion Siek, Valentino Trok">
    <link href="https://fonts.googleapis.com/css?family=Expletus+Sans" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Open+Sans" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Quicksand" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Passion+One" rel="stylesheet">
    <link rel="stylesheet" href="http://maxcdn.bootstrapcdn.com/font-awesome/4.6.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="css/styles.css">
    <link rel="stylesheet" href="css/diensten.css">
    <link rel="stylesheet" href="css/mainpc.css">
    <link rel="icon" href="images/logo.png">
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <title>Diensten</title>
</head>
<body>
<h2 style="margin-bottom: 30px;"><a href="index.php" class="headingHackware">ID<span style="color: red">Custom</span>Hardware</a></h2>




<header>


    <div class="menu-wrap" >
        <nav class="menu">
            <ul class="clearfix">
                <li class="active"><a href="index.php">Home</a></li>
                <li>
                    <a  href="computersmain.php">Computers <span class="arrow">&#9660;</span></a>

                    <ul class="sub-menu">
                        <li><a href="bureaupcs.php">Bureau pc's</a></li>
                        <li><a href="gamepc.php">Gamepc's</a></li>
                        <li><a href="htpc.php">Htpcs</a></li>
                        <li><a href="minepc.php">Mining pc's</a></li>
                        <li><a href="#">Randapparatuur</a></li>
                    </ul>
                </li>
                <li><a href="contact.php">Contact</a></li>
                <li><a href="nieuwklant.php">Registreren</a></li>
            </ul>
        </nav>
    </div>



</header>

<div class="parallax"></div>
<div align="center" >

    <figure class="snip1529" style="float: left; height: 400px; width: 350px">
        <img src="images/pc350x400.jpg" alt="pr-sample13" style="padding-bottom: 30px"/>
        <figcaption>
            <h3>Gamepc's/multi media pc's</h3>
            <p>Stel hier je krachtige gamepc samen, helemaal naar jou eigen wensen of kies 1 van onze alvast samengestelde builds</p>
        </figcaption>
        <div class="hover"><i class="ion-android-open"></i></div>
        <a href="#"></a>
    </figure>
    <figure class="snip1529" style="float: left; margin-left: 200px; height: 400px; width: 350px">
        <img src="images/htpc.jpg" />
        <figcaption>
            <h3>HTPC's</h3>
            <p>In uw woonkamer film kijken vanaf uw eigen Home Theater PC rechtstreeks op uw tv, of gewoon een kleine compacte pc</p>
        </figcaption>
        <div class="hover"><i class="ion-android-open"></i></div>
        <a href="#"></a>
    </figure>

    <figure class="snip1529" style="float: right; height: 400px; width: 350px">
        <img src="images/miningrig2.jpg" alt="pr-sample13" />
        <figcaption>
            <h3>Mining pc's</h3>
            <p>Speciaal voor het "minen" van digitale valuta zoals bitcoins</p>
        </figcaption>
        <div class="hover"><i class="ion-android-open"></i></div>
        <a href="#"></a>
    </figure>
</div>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>

<br>
<br>
<div id="gradient"></div>
<div class="parallax2"></div>

<footer class="footer">

    <div class="footer-left">


        <p class="footer-links">
            <a href="index.php">Home</a>
            ·
            <a href="computersmain.php">Computers</a>
            ·
            <a href="contact.php">Contact</a>
            ·
            <a href="nieuwklant.php">Registreren</a>
        </p>

        &copy; 2017<h2 style="margin-bottom: 30px;"><a href="index.php" class="headingHackware">ID<span style="color: red">Custom</span>Hardware</a></h2>

    </div>

    <div class="footer-center">

        <div>
            <i class="fa fa-phone"></i>
            <p>079-13572468</p>
        </div>

        <div>
            <i class="fa fa-envelope"></i>
            <p><a href="mailto:support@company.com">IDcustom@hardware.nl</a></p>
        </div>

    </div>

    <div class="footer-right">

        <p class="footer-company-about">
            <span>100% tevreden garantie</span>
            Weet u niet zeker wat u nodig heeft? neem gerust contact met ons op en wij zullen u graag verder helpen!
        </p>


    </div>

</footer>

</body>
</html>

</body>
</html>